"""Weather Prediction Center Mesoscale Precipitation Discussions.
...
def handler(valid, hours):
    ...
    df = read_postgis(
        ...
        watch_confidence, concerning from mpd
        where {limiter}
        """,
        pgconn,
        geom_col="geom",
...
@router.get("/nws/wpc_mpd.{fmt}", description=__doc__)
def service(
    fmt: SupportedFormats,
    valid: datetime = Query(
        None, description="Return MPDs at valid time, default is now"
    ),
    hours: int = Query(
